...
\section{The Thin-Torus Limit of Fractional Quantum Hall Systems}
\begin{frame}
\frametitle{\insertsectionhead}
\textbf{ \large Landau levels\\}
...
\begin{itemize}[<+->]
	...
	\end{align}	
	 ...
	 \vspace{10pt}
	 \centering $\Rightarrow$ \textbf Quantum \textbf Hall \textbf Effect (QHE)
	 \vspace{10pt}
...
\begin{frame}
\frametitle{\insertsectionhead}
\textbf{ \large Nature of FQH phases\\}
\pause
\begin{itemize}[<+->]
	\item Fractional filling leaves ``space to move''  for the particles inside the Landau level \\ $\Rightarrow$ strongly correlated, topological many-body phase emerges
	\item FQH states are incompressible quantum liquids 
	\item The excited states are separated by a finite gap $\Delta_{ex}$ and behave as fractionally charged anyons
	\item For fermions, FQH states occur at odd integer fillings $\frac{1}{3}, \frac{1}{5},...$ and for bosons at even integer fillings $\frac{1}{2}, \frac{1}{4},...$ (Laughlin-type states), will feature $m$-fold degeneracy
	\item Also more exotic states at other fillings like $\frac{5}{2}$, $\frac{7}{2}$...
\end{itemize}
...
\begin{frame}
\frametitle{\insertsectionhead}
\textbf{\large The thin-torus limit \\}
...
\begin{itemize}[<+->]
	...
	\end{align}	
	\item The TT limit adiabatically connects FQH phase to the trivial CDW phase and still retains a lot of key properties (degeneracy, quasiparticle excitations, ...)
\end{itemize}
...
\section{The Thin-Torus Limit of Fractional Chern Insulators}
...
\begin{frame}
\frametitle{\insertsectionhead}
\textbf{\large Do FCIs also possess a TT limit? \\}
...
\begin{frame}
	\frametitle{\insertsectionhead}
	\textbf{ \large Changing the aspect ratio  through the hopping amplitudes\\}
	\begin{columns}[onlytextwidth,T]
		\begin{column}{.6\textwidth}
			\uncover<1->{
			The effective aspect ratio $\frac{L_x}{L_y}$ of a tight binding model can be tuned through the hopping ratio $\frac{|J_y|}{|J_x|}$! 
			}
			\uncover<2->{
			\begin{align}
			\frac{L_x}{L_y} \propto \sqrt{\frac{|J_y|}{|J_x|}} \nonumber
			\end{align}
			}
			\uncover<3->{
...
\pause
\begin{itemize}[<+->]
	\item Does an FCI state evolve to a CDW in the limit $\frac{|J_y|}{|J_x|} \to 0$?
...
\frametitle{\insertsectionhead}
\textbf{\large Methods\\}
We investigate three most common models for FCIs in cold atom settings:
\pause
\begin{itemize}[<+->]
	\item The Harper-Hofstadter model
	\item {The coupled wire model \uncover<7>{\color{red} $\leftarrow$ More details on this}	} 
	\item The Kapit-Mueller model
\end{itemize}
\pause

{\center $\Rightarrow$ Generally, FCI models allow for an analytic solution in the TT limit, much like the continuum FQH problem! Results are confirmed through exact diagonalization.\\}
\pause
\end{frame}
...
\begin{frame}
	...
	Coupled atomic wires with a synthetic field and a contact interaction.
	\pause
	\begin{columns}[onlytextwidth,T]
...
\pause
\begin{itemize}[<+->]
	\item The interaction term is projected to the lowest band (valid if $U$ is sufficiently small compared to band gap)
	...
	\item The GS at half filling is a CDW and we can derive an analytical expression for the excitation gap 
		\begin{align}
			\Delta_{ex} = 4 \frac{U  J ^\frac{1}{4} \sqrt{2\pi}}{N_y l_B} e^{-\sqrt{J} \frac{2 \pi^2}{N_y^2} } \nonumber
		\end{align}
...
\begin{itemize}[<+->]
	...
	\item The Harper-Hofstadter model yields similar results
	\item The Kapit-Mueller model as well, but the excitation gap closes very fast
\end{itemize}
\vspace{10 pt}
\pause 
