...
\vspace{-0.5em}
\captionof{figure}{{\small (a) Illustration: The exceptional phase might be realized in the top layer of a so called square-net material by adsorbing positive and negative ions to the surface \cite{SQN_Dirac_1} . (b) For {\bf orbital-selective disorder}, the Dirac cones may split into topologically stable EPs exhibiting a characteristic square-root-like dispersion (absolute value of complex energy is shown). (c) For a sample of mesoscopic size, the vanishing zero-energy transmission seen in the clean regime (left panel) may be promoted to a finite value (right panel).} \label{FIG:illustration_setup}}
\vspace{-2em}
\end{tcolorbox}
\vspace{-0.5em}
\end{minipage}
...
\begin{minipage}{\linewidth}
{\includegraphics[trim={0cm 0cm 0cm 0.3cm}, width=0.95\linewidth]{Spectrum_He.png}}\\
\vspace{-2em}
\begin{tcolorbox}[width=\linewidth, colback=white!95!black, colframe=white, arc=3mm, boxrule=0mm, left=5pt,right=5pt]
\vspace{-0.5em}
\captionof{figure}{\small (a) Complex spectrum of $H_e(k)$ in the exceptional phase. Nodal points of the clean are split into EPs connected by {\bf Fermi arcs} (marked by red lines). Lines of {\bf vanishing imaginary part} (marked in green) entail propagating quasiparticles with {\bf infinite lifetime}. The group velocity along the green lines is parallel to the  $k_\mathrm{x}$-axis, which leads to a directional dependence of transport signatures. Disorder parameters from Eq.~(\ref{EQN:disorder}) are $\alpha =1.5$, $s_0 = 1.0$, $\gamma= 1$, $\phi = 0$.} \label{FIG:spectrum}
\vspace{-2em}
\end{tcolorbox}
\vspace{-0.5em}
\end{minipage}
...
\begin{itemize}
\item Our model system $H = H_0 + V$ consists of a base model $H_0$ featuring a {\bf Dirac dispersion} to which we add a random disorder term $V$. The terms read 
\vspace{-0.5em}
\begin{align}
...
with  $t_x = t_y = 0.5$, $s_0, \gamma, \phi \in \mathbbm R$ and the uncorrelated random amplitudes $\{ a_{\bm j} \}$ drawn from a box distribution on the real interval $[- \alpha, \alpha]$, so the overall disorder strength scales with $\alpha$.

\item Impurity scattering in the disordered system $H = H_0 + V$ gives a finite life-time to the Bloch states of $H_0$, which can be discribed through a {\bf non-Hermitian self-energy correction} $\Sigma(\bm k, \omega)$ arising from a disorder-averaged Green's function. We take $H_e(\bm k) = H_0 + \Sigma(\bm k, \omega = 0)$. The disorder parameters $s_0 = 1$, $\gamma = 0$ yield a trivial disorder potential and also a trivial self-energy correction, while $s_0 = 1.0$, $\gamma= 1$, $\phi = 0$ restrict the disorder potential to one orbital and induce an exceptional phase (see Fig.~\ref{FIG:spectrum}). 
...
B. Michen, and J. Budich, {\em Mesoscopic transport signatures of disorder-induced non-Hermitian phases}, \href{https://journals.aps.org/prb/abstract/10.1103/PhysRevB.101.165121}{Phys. Rev. Research}.
...
\begin{tcolorbox}[width=\linewidth, colback=white!95!black, colframe=white, arc=3mm, boxrule=0mm, left=5pt,right=5pt]
\vspace{-0.5em}
\captionof{figure}{\small Zero-energy conductance in $x$-direction of a finite sample of $N_\mathrm{x} = 100$, $N_\mathrm{y} = 200$ sites as a function of disorder strength $\alpha$. Disorder parameters from Eq.~(\ref{EQN:disorder})  are $s_0 = 1$, $\phi = 0$ for all curves, while we vary $\gamma = 1$, $0.9$ and $0.8$ to study deviations from perfectly orbital-selective disorder ($\gamma=1$). } \label{FIG:transport}
\vspace{-1.9em}
\end{tcolorbox}
\end{minipage}
\vspace{-0.5em}
...
\item The effect persists for small deviations from perfectly orbital-selective disorder.
\end{itemize}
}

\headerbox{Summary and Outlook}
{name=sum,span=1,column=2,below=cond_dis}{
...
\item {\bf Orbital-selective disorder} will generally permit the {\bf survival of Bloch modes} from the clean system (the green lines with zero imaginary energy in Fig.~\ref{FIG:spectrum}).
\end{itemize}
\vspace{-1em}
}
\end{poster}
