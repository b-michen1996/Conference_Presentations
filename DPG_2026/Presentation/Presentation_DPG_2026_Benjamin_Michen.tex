...
\documentclass[ddcfooter, aspectratio=169]{beamer}
...
\usepackage{tikz}
\usetikzlibrary{tikzmark, arrows.meta,math}
...
%------------------------------------------------------------------------
% Match gray of the slide title to that of the title boxes
\setbeamertemplate{frametitle}{ \nointerlineskip \begin{beamercolorbox}[wd=\paperwidth, ht=0.6cm, dp=0.2cm, leftskip=0.5cm]{title} \usebeamerfont{title}\insertframetitle \end{beamercolorbox} }
...
%  Insert slide with subsection name before each subsection
\AtBeginSubsection[]{
\begin{frame}
\vfill
\centering
...
%\setbeamercolor{section title}{bg=blue, fg = white}
...
\draw[->,>=stealth, gray, line width=0.2mm] (0.75 \linewidth, 1.4) -- (0.7 \linewidth, 1.2) 
node[pos=0, right, text=gray, align=left, font=\tiny\linespread{0.8}\selectfont]{Accumulated Berry curvature\\ up to Fermi energy $E_F$};
\end{tikzpicture}
...
%\vspace{0.5em}
	...
	\end{figure}\footnote[0]{\tiny D. E. Khmelnitskii, \em{Quantization of Hall conductivity}, 
\href{http://jetpletters.ru/ps/1485/article_22668.shtml}{ZhETF Pisma Redaktsiiu {\bfseries{38}} (1983)}}
\begin{tikzpicture}[remember picture, overlay]
\draw[->,>=stealth, gray, line width=0.2mm] (0.65 \linewidth, 1.) -- (0.625 \linewidth, 1.5) 
node[pos=0.1, below right, text=gray, align=left, font=\tiny\linespread{0.8}\selectfont]{Stable fixed point};
\end{tikzpicture}
\vspace{-1.5em}
\begin{itemize}[<+(1)->]
...
\item $S$-matrix elements calculated with Kwant\footnote{\tiny C. W. Groth, M. Wimmer, A. R. Akhmerov, X. Waintal, {\em Kwant: a software package for quantum transport}, \href{https://iopscience.iop.org/article/10.1088/1367-2630/16/6/063065}{New J. Phys. 16, 063065 (2014)}} package
\end{itemize}
...
\end{align*}
\begin{itemize}[<+(1)->]
\item Both bands of $\hat H_0$ have Chern number zero but a strong anomalous Hall effect.
\item Random disorder $\hat W$ will nudge system into quantum Hall phase!
\end{itemize}
\footnote[0]{\tiny For completeness: explicit Bloch Hamiltonian is $h_0(\bm k) = \bm d(\bm k) \cdot \bm \sigma $ with coefficients $d_x(\bm k) = \gamma \sin(k_x)$, 
$d_y(\bm k) = \lambda(k_x) \sin(k_y)$,
$d_z(\bm k) = \gamma_2[r - \cos(2 k_x)] - \lambda(k_x) \cos(k_y)$, where $\lambda(k_x) =  \epsilon_1 + \epsilon_2 (1 - \cos(k_x))/2 $ and parameters $r = 1.5$, $\epsilon_1 = 0.3$, $\epsilon_2 = 2$, $\gamma  =2$, and $\gamma_2 = 0.3$.} 
\begin{tikzpicture}[remember picture, overlay]
\draw[->,>=stealth, gray, line width=0.2mm] (0.45 \linewidth, 4.3) -- (0.4 \linewidth, 3.9) 
node[pos=0, right, text=gray, align=left, font=\tiny\linespread{0.8}\selectfont]{Two trivial bands, \\ strong anomalous\\ Hall effect};
\draw[->,>=stealth, gray, line width=0.2mm] (0.725 \linewidth, 4.3) -- (0.675 \linewidth, 3.9) 
node[pos = 0, right, text=gray] {\tiny Random amplitudes $f_{\bm j, \alpha} \in [-1,1]$};
\end{tikzpicture}
\vspace{0.5em}
\end{frame}
...
\begin{tikzpicture}[remember picture, overlay]
  \draw[->,>=stealth, blue, line width=0.2mm]
    (0.04\linewidth, 3.9) -- (0.09\linewidth, 4.3)
    node[pos=0.2, below left, text=blue, align=left,
         font=\tiny\linespread{0.8}\selectfont]{Clean Hall\\conductance};

  \draw[->,>=stealth, blue, line width=0.2mm]
    (0.755\linewidth, 3.7) -- (0.755\linewidth, 3.3)
    node[pos=0.1, above, text=blue, align=left,
         font=\tiny\linespread{0.8}\selectfont]{Disordered Hall\\conductance};

  \draw[->,>=stealth, blue, line width=0.2mm]
    (0.6\linewidth, 3.18) -- (0.6\linewidth, 2.73)
    node[pos=0.1, above, text=blue, align=left,
         font=\tiny\linespread{0.8}\selectfont]{Clean Hall\\conductance};
         
\node[draw=blue, ellipse, minimum width=0.4cm, minimum height=1cm] (myellipse) at (0.86\linewidth, 3.92) {};
\draw[-, blue, line width=0.2mm]
(myellipse.east) -- ++(.5cm, -0.3cm)
node[pos=1, right, text=blue, align=left,
font=\tiny\linespread{0.8}\selectfont]{Hall plateau};
...
\end{center}
\end{block}
\footnote[0]{\tiny Contact: \url{benjamin.michen@tu-dresden.de}}
