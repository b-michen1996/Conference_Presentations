...
\section{Some Facts on the Quantum Hall Effect}
...
%\vspace{0.5em}
	\begin{figure}[htp!]	 
			\centering 
			\includegraphics[trim={0cm 0cm 0cm 0.5cm}, width = 0.9 \linewidth]{Illustrations/2_param_scaling_DPG.png} 			
...
\item Non-quantized values of $\sigma_{xy}$ are rounded to the nearest integer!
\end{itemize}
...
\begin{frame}
\frametitle{\insertsectionhead}
\textbf{\large Model and methods} \\
\begin{align*}
\hat H = \underbrace{\sum_{\bm k} \bm c_{\bm k}^\dagger \left[ \bm d(\bm k) \cdot \bm \sigma \right] \bm c_{\bm k}}_{\hat H_0} \;+ \;\underbrace{W\sum_{j_x, j_y} \sum_{\alpha = a,b} f_{\bm j, \alpha} c_{\bm j, \alpha}^\dagger c_{\bm j, \alpha}}_{\hat W},
\end{align*}

\begin{itemize}[<+(1)->]
\item Periodic part $\hat H_0$ exhibits trivial bands\footnote{For completeness:  $\hat H_0$ determined by coefficients $d_x(\bm k) = \gamma \sin(k_x)$, 
$d_y(\bm k) = \lambda(k_x) \sin(k_y)$,
$d_z(\bm k) = \gamma_2[r - \cos(2 k_x)] - \lambda(k_x) \cos(k_y)$, where $\lambda(k_x) =  \epsilon_1 + \epsilon_2 (1 - \cos(k_x))/2 $ and parameters $r = 1.5$, $\epsilon_1 = 0.3$, $\epsilon_2 = 2$, $\gamma  =2$, and $\gamma_2 = 0.3$.} but strong anomalous Hall effect! 
\item Disorder potential $\hat W$ with overall strength $W \ge 0$ and random amplitudes $f_{\bm j, \alpha}$ drawn uniformly from $[-1,1]$.
\item Transport properties of the model are probed using the Kwant package\footnote{C. W. Groth, M. Wimmer, A. R. Akhmerov, X. Waintal, {\em Kwant: a software package for quantum transport}, \href{https://iopscience.iop.org/article/10.1088/1367-2630/16/6/063065}{New J. Phys. 16, 063065 (2014)}}.
\end{itemize}
\vspace{0.5em}
\end{frame}
...
\section{Boundary conditions and finite-size effects}
